...
internal sealed class CombineLatest10Observable<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, TResult> : IObservable<TResult>
{
    ...
    private sealed class Subscription : IDisposable
    {
        ...
        public Subscription(IObserver<TResult> observer, Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, TResult> resultSelector)
        {
            _observer = observer;
        ...
        {
            if (_has1 && _has2 && _has3 && _has4 && _has5 && _has6 && _has7 && _has8 && _has9 && _has10)
            {
