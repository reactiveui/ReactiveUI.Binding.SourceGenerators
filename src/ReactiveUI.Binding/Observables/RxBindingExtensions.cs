...
public static class RxBindingExtensions
{
    ...
    public static IObservable<T> Switch<T>(this IObservable<IObservable<T>> source) => new SwitchObservable<T>(source);
    ...
    /// <summary>
    /// An observer that delegates <see cref="IObserver{T}.OnNext"/> to an <see cref="Action{T}"/>.
    /// Errors and completion are intentionally ignored for binding scenarios.
    /// </summary>
    /// <typeparam name="T">The element type.</typeparam>
