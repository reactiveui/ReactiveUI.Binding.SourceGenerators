...
/// <summary>Lightweight CombineLatest observable that combines the latest values from 6 source observables.</summary>
...
internal sealed class CombineLatest6Observable<T1, T2, T3, T4, T5, T6, TResult> : IObservable<TResult>
{
    ...
    /// <summary>Initializes a new instance of the <see cref="CombineLatest6Observable{T1, T2, T3, T4, T5, T6, TResult}"/> class.</summary>
    ...
    public CombineLatest6Observable(
    ...
    /// <summary>Manages the active subscriptions to all six source observables and emits combined results.</summary>
    /// <param name="observer">The downstream observer.</param>
    /// <param name="resultSelector">The function to combine the latest values.</param>
    private sealed class Subscription(IObserver<TResult> observer, Func<T1, T2, T3, T4, T5, T6, TResult> resultSelector) : IDisposable
    {
        ...
        /// <summary>The function to combine the latest values from all sources into a result.</summary>
        private readonly Func<T1, T2, T3, T4, T5, T6, TResult> _resultSelector = resultSelector;

        /// <summary>The array of inner source subscriptions.</summary>
        private readonly IDisposable?[] _subscriptions = new IDisposable?[6];
        ...
        /// <summary>Whether source 3 has emitted at least one value.</summary>
        private bool _has3;

        /// <summary>Whether source 4 has emitted at least one value.</summary>
        private bool _has4;

        /// <summary>Whether source 5 has emitted at least one value.</summary>
        private bool _has5;

        /// <summary>Whether source 6 has emitted at least one value.</summary>
        private bool _has6;
        ...
        private void TryEmit()
        {
            ...
            Volatile.Read(ref _observer)?.OnNext(_resultSelector(_value1, _value2, _value3, _value4, _value5, _value6));
        }
