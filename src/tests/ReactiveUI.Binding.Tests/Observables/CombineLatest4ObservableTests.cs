...
/// <summary>Unit tests for <see cref="CombineLatest4Observable{T1, T2, T3, T4, TResult}"/> edge cases.</summary>
public class CombineLatest4ObservableTests
{
    ...
    private const int ExpectedSum = 10;
    ...
    {
        var act = static () => CombineLatestObservable.Create<int, int, int, int, int>(
            new Subject<int>(),
            new Subject<int>(),
            new Subject<int>(),
            new Subject<int>(),
