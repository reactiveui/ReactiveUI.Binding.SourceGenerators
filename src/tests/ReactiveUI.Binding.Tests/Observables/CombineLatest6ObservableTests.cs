...
/// <summary>Unit tests for <see cref="CombineLatest6Observable{T1, T2, T3, T4, T5, T6, TResult}"/> edge cases.</summary>
public class CombineLatest6ObservableTests
{
    ...
    public async Task Constructor_NullSource1_Throws()
    {
        var act = static () => CombineLatestObservable.Create(
            ...
            new Subject<int>(),
            new Subject<int>(),
            (Func<int, int, int, int, int, int, int>)(static (a, b, c, d, e, f) => a + b + c + d + e + f));
