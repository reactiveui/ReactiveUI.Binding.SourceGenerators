        ...
        IncrementalValueProvider<bool> supportsCallerArgExpr)
    {
        var invocations = context.SyntaxProvider
            .CreateSyntaxProvider(
                predicate: static (node, ct) => RoslynHelpers.IsBindToInvocation(node, ct),
                transform: BindToExtractor.ExtractBindToInvocation)
            .Where(static x => x is not null)
            .Select(static (x, _) => x!);

        var combined = invocations.Collect().Combine(supportsCallerArgExpr);
