...
/// <summary>Runs a command each time a <see cref="ButtonControl"/> is clicked. The runtime path finds a control's binder here.</summary>
public sealed class ButtonClickCommandBinder : ICreatesCommandBinding
{
    ...
    /// <inheritdoc/>
    public int GetAffinityForObject<T>(bool hasEventTarget) => typeof(T) == typeof(ButtonControl) ? ButtonAffinity : 0;

    /// <inheritdoc/>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public IDisposable? BindCommandToObject<T>(ICommand? command, T? target, IObservable<object?> commandParameter)
        where T : class => Attach(command, target as ButtonControl, commandParameter);
    ...
    public IDisposable? BindCommandToObject<T, TEventArgs>(ICommand? command, T? target, IObservable<object?> commandParameter, string eventName)
        ...
        eventName == ClickEventName
            ? Attach(command, target as ButtonControl, commandParameter)
    ...
    private static ClickAttachment? Attach(ICommand? command, ButtonControl? button, IObservable<object?> commandParameter) =>
    ...
    private sealed class ClickAttachment : IDisposable
    {
        ...
        /// <param name="button">The button to follow.</param>
        /// <param name="commandParameter">The values offered as the command parameter.</param>
        public ClickAttachment(ICommand command, ButtonControl button, IObservable<object?> commandParameter)
        {
            _command = command;
            _button = button;
