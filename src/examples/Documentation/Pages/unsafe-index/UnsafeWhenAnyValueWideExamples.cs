...
/// <summary>Demonstrates <c>WhenAnyValueUnsafe</c> over three to sixteen properties as a group of values; the paths are built while the app runs.</summary>
public static class UnsafeWhenAnyValueWideExamples
{
    ...
    public static void ObserveThreeColumns()
    {
        ...
        var changes = item.WhenAnyValueUnsafe(title, notes, notes);

        using var recording = changes.Record();

        item.Title = RenamedTitle;

        SampleCheck.Equal(new(RenamedTitle, SeededNotes, SeededNotes), recording.Latest);
    }
    ...
    public static void ObserveFourColumns()
    {
        ...
        var title = TitleColumn();
        var notes = NotesColumn();
        var changes = item.WhenAnyValueUnsafe(title, notes, notes, notes);
    ...
    public static void ObserveFiveColumns()
    {
        ...
        var title = TitleColumn();
        var notes = NotesColumn();
        var changes = item.WhenAnyValueUnsafe(
            title,
            notes,
            notes,
            notes,
