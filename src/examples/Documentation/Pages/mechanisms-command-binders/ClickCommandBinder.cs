...
/// <summary>
/// Runs a command when a <see cref="ButtonControl"/> raises <see cref="ButtonControl.Click"/>. The binder passes
/// the latest command parameter to the command, enables the button only while the command can run, and never
/// touches the button's own <see cref="ButtonControl.Command"/> property.
/// </summary>
[System.Diagnostics.DebuggerDisplay("BindCount = {BindCount}, LastEventName = {LastEventName}")]
public sealed class ClickCommandBinder : ICreatesCommandBinding
{
    ...
    /// <summary>Gets the event name of the latest attachment that named its event, or <see langword="null"/> when none did.</summary>
    public string? LastEventName { get; private set; }

    /// <summary>Gets the event argument type of the latest attachment that named its event, or <see langword="null"/> when none did.</summary>
    public Type? LastEventArgsType { get; private set; }

    /// <inheritdoc/>
    public int GetAffinityForObject<T>(bool hasEventTarget) =>
        typeof(ButtonControl).IsAssignableFrom(typeof(T)) ? BindingAffinity.ExactType : 0;
    ...
    public IDisposable? BindCommandToObject<T, TEventArgs>(ICommand? command, T? target, IObservable<object?> commandParameter, string eventName)
        where T : class
    {
        if (eventName != nameof(ButtonControl.Click))
        {
            return null;
        }

        LastEventName = eventName;
        LastEventArgsType = typeof(TEventArgs);
    ...
    {
        if (command is null || target is not ButtonControl button)
        {
            return null;
        }
    ...
    private ClickBinding? AttachToClick(ICommand? command, object? target, IObservable<object?> commandParameter)
    {
        if (command is null || target is not ButtonControl button)
        {
            return null;
        ...
        ClickBinding binding = new(button, command, commandParameter);
        button.Click += binding.OnClick;
        binding.WhenDisposed(() => button.Click -= binding.OnClick);
        return binding;
    }
    ...
    private sealed class ClickBinding : IDisposable
    {
        ...
        public ClickBinding(ButtonControl button, ICommand command, IObservable<object?> parameters)
        {
            _button = button;
            _command = command;
