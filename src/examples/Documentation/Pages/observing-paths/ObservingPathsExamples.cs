...
public static class ObservingPathsExamples
{
    ...
    /// <summary>How many scores Chloe has recorded across her courses.</summary>
    private const int ChloeGradeCount = 1;

    /// <summary>How many scores Aisha has recorded before the teacher adds one.</summary>
    private const int AishaGradeCount = 2;

    /// <summary>How many scores Aisha has recorded after the teacher adds one.</summary>
    private const int AishaGradeCountAfterRecording = 3;
    ...
    /// <summary>The key of the launch banner.</summary>
    private const string LaunchBannerKey = "photos/2026/launch-banner.png";
    ...
    /// <summary>The key of the team offsite photo.</summary>
    private const string TeamOffsiteKey = "photos/2026/team-offsite.jpg";
    ...
    {
        var board = await OpenWebshopBoardAsync().ConfigureAwait(false);
        ...
        using (board.WhenChanged(x => x.SelectedIssue!.Assignee!.Login).Subscribe(logins.Add))
        {
            ...
            SampleCheck.Equal(0, logins.Count);
        ...
        SampleCheck.SequenceEqual([PriyaLogin, TomasLogin], logins);
    }
    ...
    public static async Task ReplaceAssigneeInTheMiddleOfThePath()
    {
        ...
        SampleCheck.SequenceEqual([PriyaLogin, TomasLogin, MariaLogin], logins);
    }
    ...
    public static async Task NullInTheMiddleOfThePathEmitsNothing()
    {
        ...
        SampleCheck.SequenceEqual([TomasLogin, MariaLogin], logins);
    }
    ...
    public static async Task NullFinalValueIsEmitted()
    {
        ...
        using (board.WhenChanged(x => x.SelectedIssue!.Assignee!).Subscribe(assignees.Add))
        {
            board.SelectedIssue = giftCards;
        ...
        SampleCheck.SequenceEqual([null, tomas, null], assignees);
    }
    ...
    {
        var gradebook = await OpenIntroductionToProgrammingAsync().ConfigureAwait(false);
        ...
        using (gradebook.WhenChanged(x => x.SelectedStudent!.Enrolments).Subscribe(enrolments => gradeCounts.Add(CountGrades(enrolments))))
        {
            ...
            gradebook.RecordGradeCommand.Execute(null);
            await gradebook.RecordGradeCommand.Completion.ConfigureAwait(false);
        }

        // Recording a grade replaces the student's enrolments, and the replacement reaches the subscriber.
        SampleCheck.SequenceEqual([ChloeGradeCount, AishaGradeCount, AishaGradeCountAfterRecording], gradeCounts);
    }
    ...
    {
        var transfer = await OpenTransferScreenAsync().ConfigureAwait(false);
        var everyday = transfer.Accounts[EverydayAccountIndex];
        var savings = transfer.Accounts[SavingsAccountIndex];
        var everydayAvailable = everyday.AvailableBalance;
        var savingsAvailable = savings.AvailableBalance;
        ...
        using (transfer.WhenChanged(x => x.Draft.Source!.AvailableBalance).Subscribe(available.Add))
        {
            ...
            var everydayAfterRent = everyday.AvailableBalance;
            ...
            SampleCheck.SequenceEqual([everydayAvailable, everydayAfterRent, savingsAvailable, savings.AvailableBalance], available);
        }
    }
    ...
    {
        var browser = await OpenMediaBucketAsync().ConfigureAwait(false);
        ...
        SampleCheck.SequenceEqual([LaunchBannerKey, TeamOffsiteKey, RenamedLaunchBannerKey], keys);
    }
    ...
    private static async Task<IssueBoardViewModel> OpenWebshopBoardAsync()
    {
        ...
        await board.SignInCommand.Completion.ConfigureAwait(false);

        board.SelectedRepository = board.Repositories[0];
        board.LoadIssuesCommand.Execute(null);
        await board.LoadIssuesCommand.Completion.ConfigureAwait(false);
    ...
    private static async Task<GradebookViewModel> OpenIntroductionToProgrammingAsync()
    {
        ...
        gradebook.LoadCoursesCommand.Execute(null);
        await gradebook.LoadCoursesCommand.Completion.ConfigureAwait(false);

        gradebook.SelectedCourse = gradebook.Courses[0];
        gradebook.OpenCourseCommand.Execute(null);
        await gradebook.OpenCourseCommand.Completion.ConfigureAwait(false);
    ...
    private static async Task<TransferViewModel> OpenTransferScreenAsync()
    {
        ...
        transfer.LoadCommand.Execute(null);
        await transfer.LoadCommand.Completion.ConfigureAwait(false);
    ...
    private static Issue FindIssue(IssueBoardViewModel board, int number)
    {
        ...
        throw new InvalidOperationException($"The board has no issue {number}.");
    }
    ...
    {
        foreach (var student in gradebook.Roster)
        {
            if (student.Id == id)
            {
                return student;
        ...
        throw new InvalidOperationException($"The roster has no student {id}.");
    }
