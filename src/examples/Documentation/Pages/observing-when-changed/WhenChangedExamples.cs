...
namespace ReactiveUI.Binding.Documentation.ObservingWhenChanged;
...
public static class WhenChangedExamples
{
    ...
    public static void ObserveTodoTitle()
    {
        ...
        using var titles = item.WhenChanged(x => x.Title).Record();
    ...
    public static void ProjectTodoChecklistLine()
    {
        ...
        using var lines = item.WhenChanged(x => x.Title, x => x.IsDone, static (title, isDone) => (isDone ? DoneMark : OpenMark) + title).Record();
    ...
    public static void ObserveTodoDeadline()
    {
        ...
        using var seen = item.WhenChanged(x => x.Title, x => x.Notes, x => x.IsDone, x => x.DueDate).Record();
    ...
    public static void ObserveTodoWithPriority()
    {
        ...
        using var seen = item.WhenChanged(x => x.Title, x => x.Notes, x => x.IsDone, x => x.DueDate, x => x.Priority).Record();
    ...
    public static void ObserveWholeTodo()
    {
        ...
        using var seen = item.WhenChanged(x => x.Title, x => x.Notes, x => x.IsDone, x => x.DueDate, x => x.Priority, x => x.Tags).Record();
    ...
    public static void ObserveIssueAssignee()
    {
        ...
        using var assignees = issue.WhenChanged(x => x.Assignee).Record();

        issue.Assignee = null;
        issue.Assignee = _tomas;

        SampleCheck.SequenceEqual([_priya, null, _tomas], assignees.Values);
    }
    ...
    public static void ObserveIssueSummary()
    {
        ...
        using var seen = issue.WhenChanged(x => x.Number, x => x.Title, x => x.State, x => x.Author, x => x.Assignee!, x => x.Labels, x => x.UpdatedAt).Record();
    ...
    public static async Task ObserveIssueBoardErrorMessage()
    {
        ...
        SampleCheck.Equal(AfterOneChange, seen.Count);
        SampleCheck.Equal(RenamedIssueTitle, seen.Latest.Property5);
    }
