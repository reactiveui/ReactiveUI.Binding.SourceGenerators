...
public static class SourceAnalysis
{
    ...
    /// <summary>Compiles source text as a build with a root namespace would and runs an analyzer over it.</summary>
    /// <param name="source">The C# source to compile.</param>
    /// <param name="analyzer">The analyzer to run.</param>
    /// <param name="parseOptions">The language version and features the project compiles with.</param>
        ...
        string rootNamespace)
    {
        ...
        AnalyzerOptions options = new([], new RootNamespaceOptionsProvider(rootNamespace));
        var reported = await compilation.WithAnalyzers([analyzer], options).GetAnalyzerDiagnosticsAsync().ConfigureAwait(false);
