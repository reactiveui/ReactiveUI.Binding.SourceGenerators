...
/// <summary>Lightweight CombineLatest observable that combines the latest values from 4 source observables.</summary>
...
internal sealed class CombineLatest4Observable<T1, T2, T3, T4, TResult> : IObservable<TResult>
{
    ...
    /// <summary>Initializes a new instance of the <see cref="CombineLatest4Observable{T1, T2, T3, T4, TResult}"/> class.</summary>
    ...
    public CombineLatest4Observable(
    ...
    /// <summary>Manages the active subscriptions to all four source observables and emits combined results.</summary>
    /// <param name="observer">The downstream observer.</param>
    /// <param name="resultSelector">The function to combine the latest values.</param>
    private sealed class Subscription(IObserver<TResult> observer, Func<T1, T2, T3, T4, TResult> resultSelector) : IDisposable
    {
        ...
        /// <summary>The function to combine the latest values from all sources into a result.</summary>
        private readonly Func<T1, T2, T3, T4, TResult> _resultSelector = resultSelector;

        /// <summary>The array of inner source subscriptions.</summary>
        private readonly IDisposable?[] _subscriptions = new IDisposable?[4];
        ...
        private void TryEmit()
        {
            ...
            Volatile.Read(ref _observer)?.OnNext(_resultSelector(_value1, _value2, _value3, _value4));
        }
