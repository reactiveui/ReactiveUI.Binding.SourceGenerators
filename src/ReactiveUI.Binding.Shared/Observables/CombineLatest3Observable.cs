...
/// <summary>Lightweight CombineLatest observable that combines the latest values from 3 source observables.</summary>
...
internal sealed class CombineLatest3Observable<T1, T2, T3, TResult> : IObservable<TResult>
{
    ...
    /// <summary>Initializes a new instance of the <see cref="CombineLatest3Observable{T1, T2, T3, TResult}"/> class.</summary>
    ...
    /// <summary>Manages the active subscriptions to all three source observables and emits combined results.</summary>
    /// <param name="observer">The downstream observer.</param>
    /// <param name="resultSelector">The function to combine the latest values.</param>
    private sealed class Subscription(IObserver<TResult> observer, Func<T1, T2, T3, TResult> resultSelector) : IDisposable
    {
        /// <summary>The subscription array index for source 3.</summary>
        private const int Source3Index = 2;

        /// <summary>The function to combine the latest values from all sources into a result.</summary>
        private readonly Func<T1, T2, T3, TResult> _resultSelector = resultSelector;
        ...
        private void TryEmit()
        {
            ...
            Volatile.Read(ref _observer)?.OnNext(_resultSelector(_value1, _value2, _value3));
        }
